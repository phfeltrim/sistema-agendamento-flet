...
class PacientesView(ft.Container):
    ...
    def build(self):
        ...
        # Container para a lista de pacientes, para que possamos atualizá-lo separadamente
        self.patients_list_container = ft.Container(content=self.build_patients_list())

        return ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("Pacientes", style=ft.TextThemeStyle.TITLE_LARGE, weight=ft.FontWeight.BOLD),
                    ft.Row([self.search_field, self.search_btn, self.new_btn], alignment=ft.MainAxisAlignment.START, spacing=10),
                    self.patients_list_container
                ],
                spacing=20
            ),
            padding=20
    ...
    def build_patients_list(self, pacientes=None):
    ...
    def search_patients(self, e):
        ...
        if not termo:
            pacientes_filtrados = self.controller.listar()
        else:
            pacientes_filtrados = [
                p for p in self.controller.listar() 
                if termo in p.name.lower() or 
                   (p.cpf and termo in p.cpf.lower()) or 
                   (p.telefone and termo in p.telefone.lower())
            ]
        
        # Atualiza apenas o conteúdo da lista de pacientes
        self.patients_list_container.content = self.build_patients_list(pacientes_filtrados)
        self.update()
    ...
    def edit_patient(self, paciente_id):
        ...
        numero_tf = ft.TextField(label="Número", value=getattr(paciente, 'numero', ''), width=100)
        complemento_tf = ft.TextField(label="Complemento", value=getattr(paciente, 'complemento', ''), width=150)
        ...
        def fechar(_):
            dlg.open = False
            self.page.dialog = None # Limpa a referência do diálogo
            self.page.update()
        ...
        def salvar(_):
            ...
            numero = numero_tf.value.strip()
            complemento = complemento_tf.value.strip()
            ...
            paciente_data = {
                ...
                "email": email,
                "status": 1 if status else 0 # Converte booleano para inteiro
            }
            self.controller.editar(paciente_id, paciente_data)
            fechar(_)
            self.content = self.build()
            self.update()
        ...
        # IMPORTANTE: Adiciona o diálogo ao overlay da página para que ele possa ser exibido.
    ...
    def new_patient(self, e):
        ...
        try:
            ...
            numero_tf = ft.TextField(label="Número", width=100)
            complemento_tf = ft.TextField(label="Complemento", width=150)
            ...
            def salvar(_):
                ...
                if not name or not email:
                    ...
                    self.page.update()
                    return
                # status=1 (ativo por padrão), user=1 (admin)
                self.controller.adicionar(name, cpf, telefone, cep, numero, complemento, email, status=1, usuario_id=1)
                fechar(_)
            ...
            # Adiciona o diálogo ao overlay da página para que ele possa ser exibido.
            if not hasattr(self.page, 'overlay'):
                self.page.overlay = []
            self.page.overlay.append(dlg)
